...
```{r geom-tile, out.width="100%", fig.align="center"}
...
Un cas typique consiste à importer un tableau de données localisées en un point, que l'on désire localiser en format `sf` avec la fonction `st_as_sf()`. Le tableau weather, par exemple, comporte une latitude (colonne `lat`) et une longitude (colonne `lon`), spécifiées dans l'argument `coord`. Puisqu'il s'agit de données canadiennes, je suppose que les coordonnées sont projetées en format NAD83(CRSG), tel qu'utilisé par Statistique Canada et Ressources naturelles Canada (à défaut de trouver la bonne info en ce moment). Le code EPSG, spécifié sous l'argument `crs`, décrit l'ellipsoïde utilisé pour calculer les longitudes et latitudes ainsi que, s'il y a lieu, la projection (détails plus loin dans la section \@ref(section-systeme-coord)).

```{r}
library("sf")
weather_geo <- weather %>% 
  st_as_sf(coords = c("lat", "lon"), crs = 3348)
weather_geo
```
...
```{r quebec-plot, out.width="100%", fig.align="center"}
plot(quebec)
```

Si nous ne désirons que la géométrie,

```{r quebec-geometry-plot, out.width="100%", fig.align="center"}
quebec %>%
  st_geometry() %>%
...
```{r quebec-centroid-plot, out.width="100%", fig.align="center"}
quebec_point <- quebec %>% 
...
```{r quebec-radius-plot, out.width="100%", fig.align="center"}
quebec_point %>% 
  st_transform(3348) %>% 
...
```{r manhattan-plot, out.width="100%", fig.align="center"}
plot(manhattan)
```
...
```{r manhattan-lowres-plot, out.width="100%", fig.align="center"}
...
```{r manhattan-rcl-plot, out.width="100%", fig.align="center"}
...
```{r oecd_gj-plot, out.width="100%", fig.align="center"}
plot(oecd_gj)
```
...
```{r quebec-grid, out.width="100%", fig.align="center"}
quebec_grid <- quebec %>% 
...
```{r raster-operations, fig.height=3, fig.width=12, out.width="100%", fig.align="center"}
...
```{r quebec-sf-gg, out.width="100%", fig.align="center"}
ggplot(quebec) +
...
```{r iucn-north-america, out.width="100%", fig.align="center"}
...
```{r iucn-tmap, out.width="100%", fig.align="center"}
library("tmap")
library("tmaptools")
...
```{r iucn-tmap-view, out.width="100%", fig.align="center"}
tmap_mode("view")
...
```{r quebec-tmap-facet, out.width="100%", fig.align="center"}
