...
que j’utiliserai dans ce chapitre.
  ...
  - **Apprentissage supervisé** et **non-spervisé**. Si vous avez suivi
    ...
    régression linéaire. En revanche, un apprentissage non supervisé
...
En pratique, il convient de séparer un tableau de données en deux: un
...
![](11_autoapprentissage_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->
...
incluant plusieurs algorithmes qui consiste à **entraîner** le modèle
sur un échantillonnage aléatoire des données d’entraînement.

La technique la plus utilisée est le *k-fold*, où l’on sépare
aléatoirement le tableau d’entraînement en un nombre \(k\) de tableaux.
À chaque étape de la validation croisée, on calibre le modèle sur tous
les tableaux sauf un, puis on valide le modèle sur le tableau exclu. La
...
*simpliste*, c’est que un objet va ressembler à ce qui se trouve dans
son voisinage. Les KNN se basent en effet sur une métrique de distance
pour rechercher un nombre \(k\) de points situés à proximité de la
mesure. Les \(k\) points les plus proches sont retenus, \(k\) étant un
entier non nul à optimiser. Un autre paramètre parfois utilisé est la
distance maximale des voisins à considérer: un voisin trop éloigné
pourra être discarté. La réponse attribuée à la mesure est calculée à
partir de la réponse des \(k\) voisins retenus. Dans le cas d’une
régression, on utiliser généralement la moyenne. Dans le cas de la
classification, la mesure prendra la catégorie qui sera la plus présente
chez les \(k\) plus proches voisins.
...
![](11_autoapprentissage_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
...
une moyenne de zéro et àun écart-type de 1 est une approche courrament
utilisée.
...
pour obtenir la version la plus récente, i.e.
`devtools::install_github("rstudio/keras")`), puis de lancer la fonction
`install_keras()`. Dans mon cas, j’utilise conda.
...
```
    ...
    ## Sensitivity                NA   0.7378   0.4373
    ## Specificity            0.6874   0.8362   0.8217
    ## Pos Pred Value             NA   0.6299   0.8665
    ## Neg Pred Value             NA   0.8941   0.3555
    ## Prevalence             0.0000   0.2742   0.7258
    ## Detection Rate         0.0000   0.2023   0.3174
    ## Detection Prevalence   0.3126   0.3212   0.3663
    ## Balanced Accuracy          NA   0.7870   0.6295
...
Les **processus gaussiens** tirent profit des statistiques bayésiennes
...
Poser un problème par un processus gaussien, c’est se demander les
...
Une distribution **bi**normale, un cas particulier de la distribution
...
Les points sur l’axe (symbole x) conditionnés sont des échantillons
...
![](11_autoapprentissage_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
...
![](11_autoapprentissage_files/figure-gfm/unnamed-chunk-18-1.png)<!-- -->
...
générons plus d’un échantillon.
...
samples <- 50
x_3.n_draw <- matrix(ncol = samples, nrow = n-2)
for (i in 1:30) {
  x_3.n_draw[, i] <- mvrnorm(n = 1, mu = cond_mean, Sigma = cond_sd)
  lines(1:n, c(conditions_x, x_3.n_draw[, i]), col = rgb(0, 0, 0, 0.2))
}
x_3.n_draw_mean <- apply(x_3.n_draw, 1, mean)
...
Les simulations permettent de calculer des statistiques pour chacune des
dimensions.
...
Comme on devrait s’y attendre, la régression résultant de la mise en
...
![](11_autoapprentissage_files/figure-gfm/unnamed-chunk-22-1.png)<!-- -->

On pourra ajouter à ce noyau un bruit blanc, c’est-à-dire une variation
...
![](11_autoapprentissage_files/figure-gfm/unnamed-chunk-23-1.png)<!-- -->
