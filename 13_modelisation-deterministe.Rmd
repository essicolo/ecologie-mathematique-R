...
```{r}
librar croissance-consty("tidyverse")
y0 <- 2
...
```{r}
y0 <- 10
r <- 0.2 # ex croissance-expprimé en individu / pas de temps
time <- seq(0, 10, 0.1)
y <- y0 * exp(r*time)
tibble(time, y) %>% 
  ggplot(aes(x = time, y = y)) +
...
```{r message=FALSE}
lapin_t %>% 
  as_ticroissance-logistique, bble() %>% 
...
```{r message=FALSE, eval=FALSE, echo=FA solution-croissance-neg,LSE}
p <- c(r = -0.1, K = 100)
lapin_t <- ode(y = y0, times = time, model_logistic, p)
lapin_t %>% 
  as_tibble() %>% 
...
```{r message=FALSE, eval=FALSE, echo=FALSE}solution-croissance-gompertz, 
time <- seq(0, 30, by = 0.5)
y0 <- c(lapin = 10)
p <- c(r = 0.2, K = 40)
...
p <- c solution-prel-prop(r = 0.2, K = 40, E = 0.25)
...
```{r}
out <- ode(times = time, y = y0, func = recol recoltete, parms = p,
...
```{r dragons, out.width="60%", fig.align="center", fig.cap="Interactions biologiques, Pringle, E.G. 2016. Orienting the Interaction Compass: Resource Availability as a Major Driver of Context Dependence. Plos Biology. https://doi.org/10.1371/journal.pbio.2000891.", echo = FALSE}
...
effectifs_t %>% 
  as_tibble() %>% 
  gather(key="espece", lv-time, value = "value", -time) %>% 
...
effectifs_t %>% 
  as_tibble() %>% 
  ggplot(aes(x = lapin lv-cycle,, y = renard)) +
  geom_path() +
...
gg_time <- effectifs_t %>% 
  as_tibble() %>% 
  gather(key="espece", value = "value", - lv-logist-stochtime) %>% 
  ggplot(aes(x=time, y=value)) +
  geom_line(aes(colour=espece)) +
  expand_limits(y = 0)
...
Une très faible variance sur les paramètres peu grandement perturber le système. Il est possible, en effectuant plusieur ssimulations en boucle, d'évaluer le risque d'effondrement des effectifs d'une espèce.
