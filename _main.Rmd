...
```{r}
modglm_1 <- glm(worms ~ trt, cochran.wireworms, family = poisson(link="log"))
summary(modglm_1)
```
...
Enfin... selon la dicumentation. De mon côté, sur Linux, j'ai dû installer tensorflow et tensorflow-probability manuellement, dans un terminal:

```
conda create --name r-tensorflow python==3.6
anaconda3/envs/r-tensorflow/bin/pip install tensorflow
anaconda3/envs/r-tensorflow/bin/pip install tensorflow-probability
```
...
1. les régressions, les regroupements et les analyses en composantes principales peuvent avoit peu ou pas de signification
...
3. les résultats d'analyses discriminantes linéaries sont propices à être illusoires
...
$$
ilr_i = \sqrt{\frac{n_i^+ n_i^-}{n_i^+ + n_i^-}} ln \left( \frac{g \left( c_i^+ \right)}{g \left( c_i^+ \right)} \right)
$$

ou, à la ligne $i$ de la SBP, $n_i^+$ et $n_i^-$ sont respectivement le nombre de composantes au numérateur et au dénominateur, $g \left( c_i^+ \right)$ est la moyenne géométrique des composantes au numérateur et $g \left( c_i^- \right)$ est la moyenne géométrique des composantes au dénominateur.
