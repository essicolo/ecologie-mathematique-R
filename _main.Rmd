...
```{r ents, out.width="50%", fig.align="center", fig.cap="Les Ents, tiré du film le Seigneur des anneaux, qui prennent trop de temps avant de se décider - paradoxalement, les abrbres de décisions sont dotés d'algorithmes rapides.", echo = FALSE}
knitr::include_graphics("images/11_dragon.png")
```
...
par(mfrow = c(1, 2))
plot(yield_tr, predict(yield_gp, environment_tr, type="response"), main = "train")
abline(0, 1, col = "red")
plot(yield_te, predict(yield_gp, environment_te, type="response"), main = "test")
abline(0, 1, col = "red")
```

La prédiction semble bien fonctionner en entraînement comme en test. Pour une application à un cas d'étude, disons que pour mon site j'ai des variables environnementales de valeurs de 2, 80 et 0.5, et que je cherche la dose optmale. Je peux créer un tableau comprenant des environnements égaux pour chaque ligne, mais comprenant des incréments de dose, puis prédire la courbe de réponse ainsi que son incertitude.

```{r}
environment_appl <- data.frame(Dose = seq(0, 200, 5),
                               var1 = 2, var2 = 80, var3 = 0.5)
...
plot(environment_appl$Dose, yield_appl, type = "l")
lines(environment_appl$Dose, yield_appl + yield_appl_sd, col = "red")
lines(environment_appl$Dose, yield_appl - yield_appl_sd, col = "red")
```

Pour chaque incrément de dose de la courbe de rponse, il est possible de calculer un rendement économique et/ou écologique en fonction du prix de la dose pondéré par un coût environnemental, puis de soutirer une performance optimale en terme de fertilisation.
