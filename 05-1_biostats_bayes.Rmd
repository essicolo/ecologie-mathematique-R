...
Pour installer **`greta`**, vous devez préalablement installer Python, gréé des modules tensorflow et tensorflow-probability en suivant [le guide](https://greta-stats.org/articles/get_started.html). Toutefois, le guide n'a jamais bien fonctionné pour moi. Puis, **`greta`** est sans cesse mis à jour: des bogues sont corrigés, d'autres sont introduits... enfin.

Installez d'abord la version 0.3.1 de greta grâce au package **`devtools`**. 

```
devtools::install_version("greta", version = "0.3.1")
```
...
```{r, eval = FALSE}
library("greta")
reticulate::use_condaenv("r-greta") # sélectionner l'environnement conda (Python) auquel se conencter
library("DiagrammeR")
library ("bayesplot")
library("tidybayes")
