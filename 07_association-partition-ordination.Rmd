...
**Kmeans**. L'objectif des kmeans est de minimiser la distance euclédienne entre un nombre prédéfini de *k* groupes exclusifs.
...
```{r}
vare_eigenvec <- scores(vare_pca, scaling = 2, display = "species", choices = 1:(ncol(vareclr)-1))
vare_eigenvec
```
...
```{r}
dolph_scores <- scores(dolph_pca, display = "sites")
dolph_loads <- scores(dolph_pca, display = "species")
dolph_loads
...
ordiplot(scores(pcoa), type = 't', cex = 1.5)
...
ordiplot(scores(nmds), type = 't', cex = 1.5)
text(spec_scores, row.names(spec_scores), col = "red", cex = 0.75)
```
...
par(mfrow = c(1, 2))
plot(scores(pcoa_dolphin), pch = 16, col = factor(dolphin_sample$anatomy))
plot(scores(nmds_dolphin), pch = 16, col = factor(dolphin_sample$anatomy))
```
...
scaling <- 2
sites <- scores(vare_rda, display = "wa", scaling = scaling)
species <- scores(vare_rda, display = "species", scaling = scaling)
env <- scores(vare_rda, display = "reg", scaling = scaling)
...
> (1) La valeur optmiale de l'espèce sur une variable environnementale quantitative peut être obtenue en projetant l'espèce à angle droit sur al variable. (2) Une espèce se trouvant près d'une variable environnementale est susceptible de se trouver en plus grande abondance aux sites de statut `1` pour cette variable. (3) Les distances n'approximent pas la distance du $\chi^2$ (traduction adaptée de [Borcard et al. (2011)](http://www.springer.com/us/book/9781441979759)).
