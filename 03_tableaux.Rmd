...
Avec [Atom](https://atom.io/), mon éditeur texte préféré (il y en a d'[autres](https://alternativeto.net/software/atom/)), je vais dans Edit > Select Encoding et j'obtiens bien le UTF-8.
...
```{r tab-auto-complete, out.width="100%", fig.align="center", fig.cap="Autocomplétion dans RStudio.", echo = FALSE}
...
il se pourrait bien que R utilise la fonction `select` d'un autre module. Pour spécifier que vous désirez la fonctionn `select` du module **`dplyr`**, spécifiez `dplyr::select`.
...
L'opération inverse est `pivot_wider()`, avec laquelle nous sélectionnons une colonne spécifiant les nouvelles colonnes à construire (`names_from`) ainsi que les valeurs à placer dans ces colonnes (`values_-_from`).
...
Pour obtenir des statistiques à chaque ligne, mieux vaut utiliser `apply()`, tel que vu précédemment. Le point, `.`, représente le tableau dans une fonction qui n'a pas été conçu pour fonctionne de facto avec **`dplyr`**.
...
```{r tab-wickham-book, out.width="100%", fig.align="center", fig.cap="Couverture du libre de Hadley Wickham et Garrett Grolemund, Source: https://r4ds.had.co.nz", echo = FALSE}
