...
Nous devons définir nos *a priori* sur les paramètres du modèle. Nous avons l'intercept, ainsi que les pentes des effets fixes et des effets aléatoires. Nous pourrions créer un *a priori* par paramètre, ou bien séparer l'intercept des autres pentes, mais nous allons plutôt créer un jeu d'*a priori* pour les effets fixes et un autre pour les effets aléatoires.

```
coef_fixed <- normal(0, 10, dim = 8) # intercept et pentes fixes
mu <- corn_modmat[, 1:8] %*% coef_fixed
sd <- cauchy(0, 3, truncation = c(0, Inf))
distribution(lasrosas.corn_sc$yield) <- normal(mu, sd)
m <- model(coef_fixed, sd, mu)
plot(m)
```


```
#draws <- greta::mcmc(m, n_samples = 100, warmup = 10)
```


```
#mcmc_combo(draws, combo = c("dens", "trace"), pars = c("mu"))
```
